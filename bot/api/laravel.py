...
class LaravelClient:
    ...
    async def save_favorite(self, telegram_user_id: int, game: dict, target: float | None, scopes: list[dict]) -> dict:
        return await self._request("PUT", "/api/internal/telegram/favorites", json={"telegram_user_id": str(telegram_user_id), "appid": game["appid"], "game_name": game["name"], "header_image": game.get("header_image"), "target_price_rub": target, "alert": {"condition_type": "target_price", "target_value": target, "scopes": scopes}})
