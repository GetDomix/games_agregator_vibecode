...
from .api.client import LaravelApiError, LaravelClient
from .config import get_settings
from .presentation.card_renderer import render_card
from .presentation.ui import (MENU_ALERTS, MENU_FAVORITES, MENU_HELP, MENU_HOME, MENU_SEARCH,
...
def make_handlers(api: LaravelClient):
    ...
    async def scope_done(callback: CallbackQuery, state: FSMContext) -> None:
        ...
        await callback.answer()
        await callback.message.answer("Введи целевую цену в рублях. Например: <code>999</code>\nИли отправь <code>-</code>, чтобы добавить в избранное без создания или изменения ценового сигнала.", parse_mode="HTML")
    ...
    async def target(message: Message, state: FSMContext) -> None:
        ...
        if value is None:
            await message.answer("✅ Игра добавлена в избранное без создания или изменения ценового сигнала.", parse_mode="HTML")
        else:
            await message.answer(f"✅ Сохранено. Цель: <b>{value}</b>\nОтслеживаем: {labels}", parse_mode="HTML")
