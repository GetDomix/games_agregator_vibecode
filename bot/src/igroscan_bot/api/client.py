...
class LaravelClient:
    ...
    async def save_favorite(self, telegram_user_id: int, game: dict, target: float | None, scopes: list[dict]) -> dict:
        payload = {
            "telegram_user_id": str(telegram_user_id), "appid": game["appid"], "game_name": game["name"], "header_image": game.get("header_image"),
        }
        if target is not None:
            payload["target_price_rub"] = target
            payload["alert"] = {"condition_type": "target_price", "target_value": target, "scopes": scopes}
        return await self._request("PUT", "/api/internal/telegram/favorites", json=payload)
