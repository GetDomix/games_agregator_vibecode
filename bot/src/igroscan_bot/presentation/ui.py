...
def format_card_details(card: dict, favorite: dict | None) -> str:
    ...
    if favorite and favorite.get("alert"):
        ...
        lines.append(f"🔔 Alert {state}: {alert_condition_label(alert)}")
...
def format_favorites(items: list[dict]) -> str:
    ...
    for item in items[:20]:
        ...
        suffix = f" · {alert_condition_label(alert)}" if alert else ""
...
def format_alerts(items: list[dict], status: str) -> str:
    ...
    for alert in items[:20]:
        ...
        line = f"• <b>{escape(str(game.get('game_name') or 'Игра'))}</b> — {alert_condition_label(alert)}"
        if alert.get("event"):
...
def alert_condition_label(alert: dict) -> str:
    condition = alert.get("condition_type") or "target_price"
    if condition == "discount_percent":
        return f"скидка Steam от {alert.get('target_value') or '—'}%"
    if condition == "new_low":
        return "новый минимум наблюдений"
    return f"цель {price(alert.get('target_value'))}"
