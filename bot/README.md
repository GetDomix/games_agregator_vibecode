...
Расписание цен и проверку алертов запускает Laravel: `php artisan schedule:work`. Бот — второй интерфейс того же аккаунта: ищет игры через Laravel, показывает карточку цен и управляет общим избранным/alert-ами. Он не хранит отдельную копию данных и не запускает общий скан.
