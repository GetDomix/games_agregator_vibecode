...
export type Market = { marketplace: string; label: string; total_offers: number; scanned_offers: number; by_kind: KindStats[]; error?: string | null }
type Steam = { appid: number; name: string; header_image?: string | null; store_url: string; price_rub?: number | null; price_initial_rub?: number | null; discount_percent?: number; is_free?: boolean; available_in_ru?: boolean | null; note?: string | null; regional_prices?: { region: string; label: string; currency: 'RUB' | 'USD' | 'EUR' | 'KZT' | 'TRY'; amount: number; price_rub?: number | null }[] }
...
type SearchCandidate = { appid: number; name: string; tiny_image?: string | null; header_image?: string | null; price_rub?: number | null; candidate_kind?: string; release_status?: string | null; is_free?: boolean; available_in_ru?: boolean | null }
type PriceResponse = { query: string; steam: Steam | null; candidates: SearchCandidate[]; plati: Market; ggsel: Market; warnings: string[]; saved_to_history?: boolean; is_favorite?: boolean; deal?: Deal | null; refreshing?: boolean; freshness?: Freshness[] }
...
type Fav = {
  ...
  last_steam_price_rub?: number | null
  price_below_target?: boolean
  hit_price_rub?: number | null
  hit_source?: AlertScope['source']
  hit_offer_kind?: AlertScope['offer_kind']
  condition_type?: ConditionType
  target_value?: number | null
  // Dashboard previews share the favorite API shape so opening Configure keeps
  // the additive, non-persistent stored-price suggestion intact.
  suggested_target?: FavoriteItem['suggested_target']
  alert?: FavoriteItem['alert']
}
...
type SuggestItem = SearchCandidate
...
const steamCandidateImage = (appid: number) => `https://shared.akamai.steamstatic.com/store_item_assets/steam/apps/${appid}/capsule_231x87.jpg`
const repairCandidateImg = (appid: number) => (event: React.SyntheticEvent<HTMLImageElement>) => {
  const image = event.currentTarget
  const fallback = steamCandidateImage(appid)
  if (image.dataset.fallbackTried !== 'true' && image.src !== fallback) {
...
export default function App() {
  ...
  useEffect(() => {
    ...
    // The query owns the visible list. Clear the previous query immediately so
    // deleting or replacing text can never leave unrelated games on screen.
    setSuggests([])
    setSuggestStatus('searching')
    let cancelled = false
    const t = window.setTimeout(() => {
      api<{ candidates: SuggestItem[] }>(`/api/search?q=${encodeURIComponent(q)}&discover=1`)
        .then((d) => {
          if (cancelled) return
          const items = (d.candidates || []).slice(0, 20)
          setSuggests(items)
  ...
  async function runSearch(q: string, appid?: number | null, opts?: { force?: boolean; silent?: boolean }) {
    ...
    try {
      ...
      if (data.steam?.appid) {
        setSuggests((current) => current.map((candidate) => candidate.appid === data.steam?.appid
          ? {
              ...candidate,
              price_rub: data.steam.price_rub ?? null,
              is_free: data.steam.is_free,
              available_in_ru: data.steam.available_in_ru,
            }
          : candidate))
      }
  ...
  function suggestionButtons(): HTMLButtonElement[] {
    return Array.from(suggestListRef.current?.querySelectorAll<HTMLButtonElement>('[data-suggest-option]') ?? [])
  }

  function onSuggestionKeyDown(event: React.KeyboardEvent<HTMLButtonElement>) {
  ...
  async function onAuth(e: FormEvent<HTMLFormElement>) {
    ...
    // Явная валидация вместо нативной: нативная молча блокирует Enter
    // (особенно на мобильных), и пользователь не понимает, что произошло.
    if (!email || !/^\S+@\S+\.\S+$/.test(email)) {
      setAuthError(tr('Укажи корректный email.', 'Enter a valid email address.'))
      form.querySelector<HTMLInputElement>('input[name="email"]')?.focus()
      return
    }
    if (password.length < 8) {
      setAuthError(tr('Пароль не короче 8 символов.', 'Password must be at least 8 characters.'))
  ...
  const steamDisplayPrice = useMemo(
    () => result?.steam ? selectSteamDisplayPrice(result.steam, currency) : null,
    [currency, result],
  )
  ...
  useEffect(() => {
    ...
    window.addEventListener('keydown', onKey)
    // курсор сразу в email: Enter отправляет форму без лишних кликов
    ...
    <>
        ...
        {view === 'home' && (
          <>
            <section className="hero hero-search" data-reveal="home-search">
              ...
              <h1 className="search-title">{tr('Найти цену', 'Find the best price')}</h1>
              ...
              <form
                ...
                role="search"
                aria-label={tr('Поиск цены игры', 'Game price search')}
                onSubmit={(e) => {
                  e.preventDefault()
                  setSuggestOpen(false)
                  runSearch(query)
              ...
              >
                <div className="search-field search-field--suggest">
                  ...
                  <input
                    ...
                    onFocus={() => { setSuggestOpen(true); if (!query.trim()) setSuggestStatus('idle') }}
                    onKeyDown={(event) => {
                      if (event.key === 'ArrowDown' && suggestOpen) {
                        const first = suggestionButtons()[0]
                        if (first) {
                          event.preventDefault()
                          first.focus()
                        }
                    ...
                    onBlur={(event) => {
                      if (event.relatedTarget instanceof Node && suggestListRef.current?.contains(event.relatedTarget)) return
                    ...
                    enterKeyHint="search"
                    autoComplete="off"
                    aria-label={tr('Название игры', 'Game title')}
                    aria-autocomplete="list"
                    aria-controls="game-search-suggestions"
                    aria-expanded={suggestOpen && ((query.trim().length >= 2 && (suggests.length > 0 || suggestStatus !== 'idle')) || (!query.trim() && recents.some((recent) => recent.appid)))}
                  />
                  {suggestOpen && query.trim().length >= 2 && (suggests.length > 0 || suggestStatus !== 'idle') && (
                    <ul id="game-search-suggestions" ref={suggestListRef} className="suggest-list" aria-label={tr('Подходящие игры', 'Matching games')}>
                      {suggestStatus === 'searching' ? (
                        <li className="suggest-status" aria-live="polite">{tr('Ищем подходящие игры…', 'Finding matching games…')}</li>
                      ) : suggests.length === 0 ? (
                        ...
                        suggests.map((s) => (
                          <li key={s.appid}>
                            ...
                            >
                              <span className="suggest-art" aria-hidden="true"><img src={s.tiny_image || steamCandidateImage(s.appid)} alt="" onError={repairCandidateImg(s.appid)} /></span>
                              <span className="suggest-copy"><span className="suggest-name">{s.name}</span><small>{({ game: tr('Игра', 'Game'), demo: tr('Демоверсия', 'Demo'), dlc: tr('Дополнение', 'Add-on'), remaster: tr('Ремастер', 'Remaster'), edition: tr('Издание', 'Edition'), soundtrack: tr('Саундтрек', 'Soundtrack') }[s.candidate_kind || 'game'])}</small></span>
                              <span className={`suggest-price ${s.price_rub == null && s.release_status !== 'announced' ? 'muted' : ''}`}>
                                {s.release_status === 'announced'
                                  ? tr('Ещё не вышла', 'Coming soon')
                                  : s.is_free || s.price_rub === 0
                                    ? tr('Бесплатно', 'Free')
                                    : s.price_rub != null
                                      ? money(s.price_rub)
                  ...
                  {suggestOpen && !query.trim() && recents.filter((recent) => recent.appid).slice(0, 4).length > 0 && (
                    <ul id="game-search-suggestions" ref={suggestListRef} className="suggest-list" aria-label={tr('Недавние игры', 'Recent games')}>
                      {recents.filter((recent) => recent.appid).slice(0, 4).map((recent) => <li key={`${recent.q}:${recent.at}`}><button type="button" className="suggest-item suggest-item--recent" data-suggest-option onMouseDown={(event) => event.preventDefault()} onKeyDown={onSuggestionKeyDown} onClick={() => { setSuggestOpen(false); setQuery(recent.q); searchInputRef.current?.blur(); runSearch(recent.q, recent.appid) }}><span className="suggest-name">{recent.q}</span><span className="suggest-price muted">{tr('Недавнее', 'Recent')}</span></button></li>)}
                    </ul>
                  )}
                </div>
                ...
                <section className="section">
                    ...
                    <article className="hero steam-card" style={{ marginTop: 0 }}>
                      ...
                      <div>
                        <div className="steam-badge-row">
                          <span className={`badge ${result.steam.available_in_ru === true ? 'ok' : result.steam.available_in_ru === false ? 'warn' : ''}`}>
                        ...
                        <div className="price-xl">
                          {result.steam.is_free
                            ? tr('Бесплатно', 'Free')
                            : steamDisplayPrice?.kind === 'amount'
                              ? formatAmount(steamDisplayPrice.value, steamDisplayPrice.currency)
                              : money(steamDisplayPrice?.value)}
                  ...
                  {result.steam && (
                    <PriceHistory
                      ...
                      refreshKey={`${Boolean(result.refreshing)}:${(result.freshness ?? []).map((source) => source.last_success_at ?? '').join('|')}`}
                      onRequireAuth={() => {
                        setToast(tr('Журнал изменений доступен после входа.', 'Sign in to view the price change log.'))
                        setAuthTab('login')
                        setAuthOpen(true)
                      }}
                    />
            ...
            <section className="section panel about-panel">
              <button type="button" className="about-toggle" onClick={() => setAboutOpen((v) => !v)} aria-expanded={aboutOpen}>
                <h2>{tr('Зачем это нужно', 'Why use it')}</h2>
              ...
              <div className={`about-body ${aboutOpen ? 'open' : ''}`}>
                ...
                <div className="steps">
                  <div className="step">
                    <h3>{tr('Экономия времени', 'Save time')}</h3>
                  ...
                  <div className="step">
                    <h3>{tr('Понятная выгода', 'Clear value')}</h3>
                  ...
                  <div className="step">
                    <h3>{tr('Следи за ценой', 'Track prices')}</h3>
                    <p>{tr('В аккаунте доступны история, избранное и ценовые сигналы.', 'Your account keeps history, watchlist, and price alerts.')}</p>
                  </div>
                </div>
              </div>
            </section>
            ...
            {SHOW_POST_MVP_FEATURES && (
              ...
              <div className="radar-cta-copy">
                ...
                <p className="muted">
                  {tr('Радар и бот', 'Radar and bot')} <strong>@igroscan_bot</strong>: {tr('избранное, ценовые сигналы и уведомления в Telegram.', 'watchlist, price alerts, and Telegram notifications.')}
                </p>
                <button
                  type="button"
          ...
          <section className="section hero">
            ...
            <div className="steps" style={{ marginTop: '0.75rem' }}>
              ...
              <div className="step">
                ...
                <p>
                  {tr('Добавьте игру в избранное и настройте ценовой сигнал. Кабинет покажет сработавшие условия.', 'Add a game to your watchlist and configure a price alert. The dashboard will show triggered conditions.')}
                </p>
              </div>
            </div>
          ...
          <section className="section page-enter radar-page">
            ...
            {!loggedIn ? (
              <div className="panel section">
                ...
                <p className="muted">{tr('Избранное и ценовые сигналы хранятся в аккаунте.', 'Your watchlist and price alerts are stored in your account.')}</p>
              ...
              <>
                <div className="radar-steps section">
                  <article className="panel radar-step">
                    ...
                    <h3>{tr('Избранное + сигнал', 'Watchlist + alert')}</h3>
                    <p className="muted">
                      {tr('Найди игру → добавь в избранное → выбери условие и источники.', 'Find a game → add it to your watchlist → choose a condition and sources.')}
                    </p>
                ...
                <div className="panel section">
                  ...
                  <ul className="guide-list">
                    <li>
                      <strong>{tr('Своя цена', 'Custom price')}</strong> — {tr('цена выбранного предложения стала не выше заданной.', 'the selected offer price reaches or falls below your target.')}
                    </li>
                    <li>
                      <strong>{tr('Скидка Steam', 'Steam discount')}</strong> — {tr('официальная цена Steam достигла выбранного процента скидки.', 'the official Steam price reaches your selected discount percentage.')}
                    </li>
                    <li>
                      <strong>{tr('Новый минимум', 'New observed low')}</strong> — {tr('новая цена ниже всех ранее увиденных Игросканом для этого источника и типа; это не исторический минимум Steam.', 'a fresh price is below every earlier price Igroscan observed for that source and offer type; it is not an all-time Steam low.')}
                    </li>
                    <li>
            ...
            <div className="cabinet-workspace">
                      ...
                      <article key={f.appid} className="list-card cabinet-game-card">
                        ...
                        <div className="cabinet-game-content">
                          ...
                          <div className="cabinet-target-row">
                            <span>{!f.alert ? tr('Сигнал не настроен', 'Alert not configured') : f.alert.condition_type === 'discount_percent' ? tr('Скидка Steam', 'Steam discount') : f.alert.condition_type === 'new_low' ? tr('Новый минимум', 'New observed low') : tr('Целевая цена', 'Target price')}</span>
                            {f.alert?.condition_type === 'target_price' && <b>{money(f.alert?.target_value)}</b>}
                          </div>
              ...
              {SHOW_POST_MVP_FEATURES && (
                ...
                {/* TODO [Cabinet radar] Временно вырезано из MVP */}
                  ...
                  <p className="muted">
                    {!tgStatus?.linked
                      ? tr('Подключи Telegram, чтобы получать ценовые сигналы.', 'Connect Telegram to receive price alerts.')
                ...
                {dashboard?.price_hits && dashboard.price_hits.length > 0 && (
                  <div className="panel cabinet-hits">
                    <div className="panel-head">
                      <div>
                        <p className="panel-kicker">{tr('Сигнал сработал', 'Alert triggered')}</p>
                      ...
                      {dashboard.price_hits.map((f) => (
                        <article key={f.appid} className="list-card hit">
                          ...
                          <div>
                            <strong>{f.game_name}</strong>
                            <span className="offer-meta">
                              {f.hit_source && f.hit_offer_kind
                                ? scopeLabel({ source: f.hit_source, offer_kind: f.hit_offer_kind }, tr)
                                : tr('Предложение', 'Offer')}{' '}
                              {money(f.hit_price_rub)} · {f.condition_type === 'discount_percent'
                                ? tr(`скидка от ${f.target_value ?? '—'}%`, `discount from ${f.target_value ?? '—'}%`)
                                : f.condition_type === 'new_low'
                                  ? tr('новый минимум', 'new observed low')
                                  : <>{tr('цель', 'target')} {money(f.target_price_rub)}</>}
                            </span>
        ...
        {view === 'favorites' && (
          loggedIn ? (
            <section className="section page-enter">
              ...
              <p className="muted">{tr('Ценовые сигналы и уведомления настраиваются отдельно для каждой игры.', 'Price alerts and notifications are configured per game.')}</p>
              <div className="list-cards panel-list">
                {watchlist.map((f) => (
                  <article key={f.appid} className="list-card">
                    ...
                    <div>
                      ...
                      <span className="offer-meta">{!f.alert ? tr('сигнал не настроен', 'alert not configured') : f.alert.condition_type === 'discount_percent' ? tr(`скидка Steam от ${f.alert.target_value}%`, `Steam discount from ${f.alert.target_value}%`) : f.alert.condition_type === 'new_low' ? tr('новый минимум наблюдений', 'new observed low') : `${tr('цель', 'target')} ${money(f.alert.target_value)}`}{f.alert?.scopes?.length ? ` · ${f.alert.scopes.map((scope) => `${scope.source}/${scope.offer_kind}`).join(' · ')}` : ''}</span>
                      {f.release_status === 'announced' ? <span className="offer-meta">Ожидаем релиз в Steam — маркетплейсы пока не запрашиваются.</span> : null}
                      <div className="actions">
                        <button type="button" className="btn ghost sm" onClick={() => runSearch(f.game_name, f.appid)}>{tr('Цены', 'Prices')}</button>
                        {SHOW_POST_MVP_FEATURES && <button type="button" className="btn ghost sm" onClick={() => setAlertModal(f)}>{tr('Настроить', 'Configure')}</button>}
            ...
            <section className="section hero page-enter">
              ...
              <p className="lead">{tr('Избранное, ценовые сигналы и Telegram-уведомления доступны после входа.', 'Watchlist, price alerts, and Telegram notifications are available after sign-in.')}</p>
              <button type="button" className="btn primary" onClick={() => { setAuthTab('login'); setAuthOpen(true) }}>
                {tr('Войти', 'Sign in')}
              </button>
            </section>
      ...
      {SHOW_POST_MVP_FEATURES && alertModal && (
        <AlertSettingsModal
          ...
          onSave={async ({ condition_type, target_value, scopes }: { condition_type: ConditionType; target_value: number | null; scopes: AlertScope[] }) => {
            const favorite = alertModal
            await api(`/api/me/favorites/${favorite.appid}`, { method: 'PATCH', body: JSON.stringify({ alert: { condition_type, target_value, scopes } }) })
            await loadWatchlist()
            loadDashboard().catch(() => {})
            setToast(tr('Настройки алерта сохранены', 'Alert settings saved'))
          }}
          onRemoveAlert={async () => {
            await api(`/api/me/favorites/${alertModal.appid}/alert`, { method: 'DELETE' })
            setAlertModal(null)
            loadWatchlist().catch(() => {})
      ...
      <AnimatePresence>
        {authOpen && (
          <motion.div className="modal-backdrop" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} onClick={() => setAuthOpen(false)}>
            <motion.div className="modal" role="dialog" aria-modal="true" aria-labelledby="auth-modal-title" initial={{ y: 40, opacity: 0 }} animate={{ y: 0, opacity: 1 }} exit={{ y: 20, opacity: 0 }} onClick={(e) => e.stopPropagation()}>
              <div className="modal-head">
                <h2 id="auth-modal-title">{authTab === 'login' ? tr('Вход в Игроскан', 'Sign in to Igroscan') : tr('Регистрация', 'Create account')}</h2>
              ...
              <form className="auth-form" noValidate onSubmit={onAuth}>
                {authTab === 'register' && (
                  <label>
                    {tr('Имя', 'Name')}
...
export function MarketCard({ market, onTrack, steamPrice }: { market: Market; onTrack: (marketplace: string, url: string, price?: number) => void; steamPrice?: number | null }) {
  ...
  const hasStoredOffers = visibleKinds.length > 0 && market.total_offers > 0
  if (market.error && !hasStoredOffers) {
    return (
  ...
  // Empty when nothing relevant remains (ignore stale/pre-filter totals).
    ...
    return (
      <article className={`market market-body ${market.marketplace} market-empty`}>
        ...
        <p className="market-empty-note">
          {tr('Релевантных предложений по этой игре пока нет на', 'No relevant offers for this game on')} {market.label}. {tr('Похожие, но другие игры не показываем. Таблица обновится после следующего скана.', 'Similar but different games are hidden. The table will update after the next scan.')}
        </p>
      </article>
    )
  }
  return (
    <article className={`market market-body ${market.marketplace}`}>
      ...
      {market.error && (
        <p className="status warn market-stale-notice" role="status">
          {market.error}. {tr('Показана последняя успешно сохранённая цена.', 'Showing the last successfully saved price.')}
        </p>
      )}
