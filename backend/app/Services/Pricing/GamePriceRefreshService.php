...
class GamePriceRefreshService
{
    ...
    {
        $freshObservationIds = DB::transaction(function () use ($game, $state, $result) {
            $freshObservationIds = [];
            $wasReleased = $game->isReleased();
            $previousName = (string) $game->name;
            ...
            // Steam renamed the app → Plati/GGsel catalog ids may point at the wrong card.
            if ($result->source === GameSourceState::SOURCE_STEAM
                && isset($gameValues['name'])
                && mb_strtolower(trim($previousName)) !== mb_strtolower(trim((string) $gameValues['name']))) {
                $game->forceFill([
                    'plati_id_cb' => null,
            ...
            foreach ($result->offerGroups as $group) {
                ...
                $observation = GamePriceObservation::query()->create([
                    ...
                    'offer_kind' => $kind,
                    'min_price_rub' => $group['min_price_rub'],
                    'discount_percent' => $result->source === GameSourceState::SOURCE_STEAM ? $result->discountPercent : null,
                    'offer_title' => $cheapest['title'] ?? null,
                    'offer_url' => $cheapest['url'] ?? null,
                    'offer_sales' => $cheapest['sales'] ?? null,
                    'observed_at' => now(),
                ]);
                $freshObservationIds[] = $observation->id;
            }
            ...
            return $freshObservationIds;
        });

        return $this->alerts?->evaluate($game->fresh(), $result->source, $freshObservationIds) ?? 0;
    }
